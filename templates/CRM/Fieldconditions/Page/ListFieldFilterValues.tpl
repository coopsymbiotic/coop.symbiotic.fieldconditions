...
{foreach from=$values item=row}
  <tr>
    ...
    {/foreach}
    <td><a href="{crmURL p='civicrm/admin/fieldconditions/filter-values/edit' q="action=delete&reset=1&map_id=`$map_id`&id=`$row.id`"}">{ts}Delete{/ts}</a></td>
  </tr>
{/foreach}
</table>
